...
class Retention(nn.Module):
    def __init__(self, dim, n_head):
        ...
        self.qkv_proj = nn.Linear(dim, 3*dim, bias=False)
        self.o_proj = nn.Linear(dim, dim, bias=False)
        self.gn = nn.GroupNorm(num_groups=dim_head, dims=-1, affine=False)
        self._start = mx.zeros((1, n_head, dim_head, dim_head))
        self._gamma =  1 - mx.linspace(math.log(1 / 512), math.log(1 / 32), num=n_head).exp()
    def __call__(self, x, position_ids=None, attention_mask=None, cache=None, use_recurrent_mode=True):
    ...
    def recurrent_mode(self, x, cache):
        ...
        qkv = self.qkv_proj(x)
        q, k, v = mx.split(qkv, 3, axis=-1)
        q = q.reshape(B, 1, self.n_head, -1).transpose(0, 2, 1, 3)
        k = k.reshape(B, 1, self.n_head, -1).transpose(0, 2, 1, 3)
        v = v.reshape(B, 1, self.n_head, -1).transpose(0, 2, 1, 3)
        k = k * self.scale
        s = self._gamma[None, :, None, None] * s + (k.transpose(0, 1, 3, 2) @ v)
        o = q @ s
        o = o.transpose(0, 2, 1, 3).reshape(B, -1)
        o = self.gn(o).reshape(B, 1, -1)
        o = self.o_proj(o)
        return o, (s, n+1)
...
class Layer(nn.Module):
    ...
    def __call__(self, x, position_ids=None, attention_mask=None, cache=None, use_recurrent_mode=True):
        r, cache = self.self_attn(self.input_layernorm(x), position_ids, attention_mask, cache, use_recurrent_mode=use_recurrent_mode)
        h = x + r
...
class Transformer(nn.Module):
    ...
    def __call__(self, x, position_ids=None, attention_mask=None, cache=None, use_recurrent_mode=True):
        ...
        for i, l in enumerate(self.layers):
            x, cache[i] = l(x, position_ids=position_ids, attention_mask=attention_mask, cache=cache[i], use_recurrent_mode=use_recurrent_mode)
...
class Aggressor(nn.Module):
    ...
    def __call__(self, seq):
        ...
        cond, _ = self.transformer(cond_seq, use_recurrent_mode=False)
        sum_loss = 0
        step = 0
