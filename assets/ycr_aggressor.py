...
class Aggressor(nn.Module):
    def __init__(self, image_shape, n_chop, n_head, n_diff, n_loop, n_layer):
        ...
        self.n_loop = n_loop
        self._pe = mx.array(np.indices((1, num_patches))).reshape(2, -1).T
    ...
    def __call__(self, seq):
        seq = rearrange(seq, 'b (n l) c -> b (n c) l', n=self.n_chop)
        B, S, _ = seq.shape
        cond_seq = seq[:, :-1]
...
def train(model, dataset, n_epoch, batch_size, lr, postfix):
    ...
    def loss_fn(model, x):
        return model(x)
    f_name = f'{dataset.info.dataset_name}_ycr_{datetime.now().strftime("%Y%m%d_%H%M%S")}{postfix}'
